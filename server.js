...
app.get('/vendor-c', async (req,res)=>{
    // Data produk dengan struktur bersarang (Nested Object) untuk Vendor C
    const products=[
        {
            "id": 501,
            "details": {
    ...
    // Logika untuk mengolah data bersarang sebelum disimpan ke DB
    for(const p of products){
        const totalPrice = p.pricing.base_price + p.pricing.tax;
        
        // Simpan data ke vendor_products. Mengambil nilai dari nested fields.
        await pool.query(
            'INSERT INTO vendor_products(vendor,code,name,price,stock) VALUES($1,$2,$3,$4,$5)',
            [
                'VendorC', 
                p.id, // ID sebagai Kode Produk
                p.details.name, // Nama dari objek details
                totalPrice, // Harga total (base_price + tax)
                p.stock // Stok dari root
            ] 
        );
    }
    
    // Mengembalikan data JSON yang bersarang sesuai format Vendor C
    res.json(products); 
...
app.listen(3002,() => console.log("M2"));
